...
def main():
    ...
    with tabs[2]:
        ...
        try:
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT id, condition, action FROM knowledge_source")
            rules = [(str(row[0]), f"{row[1]} -> {row[2]}") for row in cursor.fetchall()]
            conn.close()
            
            if rules:
                options = {rule[1]: rule[0] for rule in rules}
                selected_rule = st.selectbox("Select rule to delete:", list(options.keys()))
                selected_id = options[selected_rule]
                
                if st.button("Delete Selected"):
                    try:
                        conn = get_connection()
                        cursor = conn.cursor()
                        cursor.execute("DELETE FROM knowledge_source WHERE id = %s", (selected_id,))
                        conn.commit()
